...
# # Create some visual to show
# im1 = io.load_crate().astype('float32') / 255
# image = scene.visuals.Image(im1, parent=vb1.scene)
# # Assign camera
# vb1.camera = scene.PanZoomCamera()

# # Put viewboxes in a grid
# grid.add_widget(vb1, 4, 0)
...
def update(ev):
    ...
    #         first += 1
        ...
        # if not valid_data % 25:
        #     # plot only last <len_buff> values
        #     print(len(daqDeque))
        #     line.set_data(pos=pos[first:valid_data])
        #     line2.set_data(pos=pos[first:valid_data])
        #     line3.set_data(pos=pos[first:valid_data])
